...
import RateTypeDefaultShareModal from "../../Frontend/util/RateTypeDefaultShareModal";
...
const RateTypeShareMaster = () => {
  ...
  const getRateTypeShare = () => {
    setLoading(true);
    axios
      .post("/api/v1/RateTypeShare/GetRateTypeShare", payload)
      .then((res) => {
        if (res.status === 200) {
  ...
  const post = () => {
    ...
    if (valid) {
      ...
      setLoad(true);
      axios
        .post("/api/v1/RateTypeShare/RateTypeShareCreate", {
          RateTypeID: payload?.RateTypeID,
          DepartmentID: payload?.DepartmentID,
          Data: data,
        })
  ...
  const DeleteData = () => {
    const data = TableData?.filter((ele) => ele.isChecked === true);
    axios
      .post("/api/v1/RateTypeShare/RateTypeShareDelete", {
        RateTypeID: payload?.RateTypeID,
        DepartmentID: payload?.DepartmentID,
        Data: data,
      })
  ...
  return (
    <div className="box box-success form-horizontal">
      ...
      <div className="box-header with-border">
        <h6 className="box-title">{t("Rate Type Share Master")}</h6>
      </div>
      <div className="box-body">
        <div className="row">
          <label className="col-sm-1">{t("RateType")}:</label>
