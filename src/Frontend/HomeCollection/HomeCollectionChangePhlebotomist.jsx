...
const HomeCollectionChangePhlebotomist = () => {
  ...
  const fetchDroplocation = async () => {
    await axios.get("api/v1/PhelebotomistMapping/GetPhelebo")
      .then((res) => {
        if (res.data.message) {
  ...
  const handleDropChange = (event, id) => {
    ...
    if (name === "dropLocation") {
      ...
      const newData = newlist.map((item) => {
        if (item.id === id) {
          return {
            ...item, PhlebotomistId: value, PheleboName: filtered[0].label
          };
        }
        return item;
      });
  ...
  const handleCheckboxChange = (id, route, dropLocation, centerID, Routename, RouteID, Phelboname, PhelbotomistID) => {
  ...
  const handleSave = () => {
    ...
    if (Object.keys(UpdatePhelebo).length === 0) {
      ...
      axios.post('api/v1/PhelebotomistMapping/UpdateHCChangePheleboData', { UpdatePhelebo })
        .then((res) => {
          setPhleboTable(null)
          handleSearch()
  ...
  const handleSearch = async () => {
    ...
    await axios.post("api/v1/PhelebotomistMapping/GetHCChangePheleboData", {
      ...
      <div className="box box-success">
        ...
        <div className="box-body" >
          <div className="row ">
            ...
            <label className="col-sm-2" htmlFor="inputEmail3" style={{ textAlign: "right" }}>
              {t("Prebooking ID")}:
            </label>
            <div className="col-sm-2">
              <Input
                      ...
                      <td data-title="#" className="text-center">
                        {index + 1} &nbsp; <input type="checkbox" onClick={
                          () => handleCheckboxChange(ele.PreBookingID,
                            ele.route, ele.dropLocation, ele.cpid, ele.RouteName,
