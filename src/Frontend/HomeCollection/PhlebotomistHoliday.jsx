...
const PhlebotomistHoliday = () => {
    ...
    const handleSelectChange = async (event) => {
        ...
        if (name === 'StateId' || name === 'CityId' || name === "Phlebotomist") {
            if (name === 'StateId') {
                fetchCities(value);
                console.log(value)
                setphleboHoliday({ ...searchData, [name]: value, CityId: "", Phlebotomist: "" });
            }
            if (name === 'CityId') {
    ...
    const deleteHoliday = (value) => {
        setLoading(true);
        axios.post("api/v1/PhelebotomistMaster/CancelPhelboHoliday", {
    ...
    const searchHoliday = async () => {
        ...
        if (generatedError === "") {
            ...
            await axios.post("api/v1/PhelebotomistMaster/GetHolidayData", {
    ...
    const saveHoliday = () => {
        ...
        setLoading(true);
        if (generatedError === "") {
            axios.post("api/v1/PhelebotomistMaster/SaveHoliDay", {
                    ...
                    <div className="row">
                        ...
                        <div className="col-sm-2">
                            <DatePicker
                                className="form-control input-sm"
                                name="FromDate"
                                date={phleboHoliday?.FromDate ? new Date(phleboHoliday?.FromDate) : new Date()}
                                onChange={dateSelect}
                                minDate={new Date()}
                            />
                        ...
                        <div className="col-sm-2">
                            <DatePicker
                                className="form-control input-sm"
                                name="ToDate"
                                date={phleboHoliday?.ToDate ? new Date(phleboHoliday?.ToDate) : new Date()}
                                                    ...
                                                    <tr key={index}>
                                                        ...
                                                        <td data-title="Cancel" className="text-center">
                                                            <button className="btn  btn-info btn-sm"
                                                                onClick={() => deleteHoliday(ele.id)}>Cancel</button>
                                                        </td>
                                                        <td data-title="Phlebotomist" className="text-center">
                                                            {ele.Name}&nbsp;
                                                        </td>
